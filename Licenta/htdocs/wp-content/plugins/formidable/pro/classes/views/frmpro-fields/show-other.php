        ...
        foreach ($previous_fields as $prev){
            if (isset($_POST['item_meta'][$prev->id]) or $prev->type == 'checkbox'){ 
                if(!isset($_POST['item_meta'][$prev->id]))
                    $_POST['item_meta'][$prev->id] = ($prev->type == 'checkbox') ? array('') : '';
                
                if (is_array($_POST['item_meta'][$prev->id])){
                    foreach (stripslashes_deep($_POST['item_meta'][$prev->id]) as $v => $checked){
                        $checked = apply_filters('frm_hidden_value', $checked, (array)$prev);
                        
                        echo '<input type="hidden" name="item_meta['. $prev->id .']['. $v .']" value="'. esc_attr($checked) .'" />'."\n";
                        unset($v);
                        unset($checked);
                    }
                }else{ ?>
<input type="<?php echo apply_filters('frm_paged_field_type', 'hidden', array('field' => $prev)) ?>" id="field_<?php echo $prev->field_key ?>" name="item_meta[<?php echo $prev->id ?>]" value="<?php echo esc_attr(stripslashes($_POST['item_meta'][$prev->id])) ?>" />
    <?php       }
            }
        } 
    }else{ ?>
